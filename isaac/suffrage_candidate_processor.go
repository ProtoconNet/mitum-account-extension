...
func (p *SuffrageCandidateProcessor) PreProcess(
	...
	e := util.StringErrorFunc("failed to preprocess SuffrageCandidateProcessor")

	fact, ok := op.Fact().(SuffrageCandidateFact)
	if !ok {
		return ctx, nil, e(nil, "expected SuffrageCandidateFact, not %T", op.Fact())
	}
...
func (p *SuffrageCandidateProcessor) Process(ctx context.Context, op base.Operation, getStateFunc base.GetStateFunc) (
	...
	e := util.StringErrorFunc("failed to process SuffrageCandidateProcessor")
	...
	fact, ok := op.Fact().(SuffrageCandidateFact)
	if !ok {
		return nil, nil, e(nil, "expected SuffrageCandidateFact, not %T", op.Fact())
	}
